...
export default function CreateListing() {
    ...
    useEffect(() => {
        if(listing && listing.userRef !== auth.currentUser.uid){
            toast.error("Usted no puede editar esta Publicación");
            navigate("/");
        }
    ...
    function onChange(e) {
        ...
        // This is for Text/Boolean/Number
        if(!e.target.files){
            setFormData((prevState)=>({
                ...prevState,
    ...
    async function onSubmit(e) {
        ...
        if(images.length > 9){
            setLoading(false)
            toast.error("Se permite un máximo de 8 imágenes");
            return;
        }
        let geolocation = {}
        let location
        if(geolocationEnabled){
            const response = await fetch(`https://maps.googleapis.com/maps/api/geocode/json?address=${address}&key=${process.env.REACT_APP_GEOCODE_API_KEY}`);    // 9:01:00       
        ...
        const imgUrls = await Promise.all(
            ...
            setLoading(false);
            toast.error("No se cargaron las Imagenes");
            return;
        });
        const formDataCopy = {
            ...formData,
        ...
        delete formDataCopy.images;                                  // LISTINGS   LISTADOS
    ...
    return (
        <main className="max-w-md px-2 mx-auto">
            <h1 className="text-3xl text-center mt-6 font-bold">Editar Anuncio</h1>
            <form onSubmit={onSubmit}>
                ...
                <input 
                    ...
                    value={name} 
                    onChange= {onChange} 
                    placeholder="Ej. Casa a estrenar en barrio privado" //Property name
                    maxLength="52"
                    minLength="10" 
